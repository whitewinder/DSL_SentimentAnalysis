...
import os
os.environ['Theano_Flags']='device=gpu%d,lib.cnmem=1,floatX=float32' % (config_params.gpu)
...
import numpy
from Util_basic import sgd_joint, prepare_data_x, unzip, itemlist_NoEmb, adadelta_joint
...
class monitor(object):
    def __init__(self):
        ...
        self.classifier = Classifier(lrate=1.,  # Learning rate for sgd (not used for adadelta and rmsprop)
                                     ...
                                     dispFreq=1,  # Display the training progress after this number of updates
                                     ...
                                     reload_model=None,
                                     embedding='./data/embedding500.npz', # Path to the word embedding file (otherwise randomized)
                                     warm_LM=None,
    ...
    def build(self):
        ...
        lambda0 = tensor.scalar(dtype='float32')
        lambda1 = tensor.scalar(dtype='float32')
        ...
        '''
        self.f_grad_shared, self.f_update = adadelta_joint(lr0, [self.CLM.tparams, self.classifier.tparams], [grads_L2S, grads_S2L],
                                              [CLM_srcx, CLM_srcx_mask, CLM_ctx_, classifier_x, classifier_mask, classifier_y, LMsores],
                                              [CLM_cost_avg, classifier_nlls_avg, consistent_loss, overall_L2S, overall_S2L, norm_grads_L2S, norm_grads_S2L])
