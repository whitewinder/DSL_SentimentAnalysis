...
import os
os.environ['THEANO_FLAGS']='device=gpu%d,floatX=float32' % (config_params.gpu)
...
import numpy
from Util_basic import sgd_joint, prepare_data_x, unzip, itemlist_NoEmb, adadelta_joint, adadelta
...
class monitor(object):
    ...
    def build(self):
        ...
        lr0 = tensor.scalar(dtype='float32')
        lr1 = tensor.scalar(dtype='float32')
        ...
        '''
        self.f_grad_shared, self.f_update = sgd_joint([lr0, lr1], [self.CLM.tparams, self.classifier.tparams], [grads_L2S, grads_S2L],
                                                      [CLM_srcx, CLM_srcx_mask, CLM_ctx_, classifier_x, classifier_mask, classifier_y, LMsores],
                                                      [CLM_cost_avg, classifier_nlls_avg, consistent_loss, overall_L2S, overall_S2L, norm_grads_L2S, norm_grads_S2L],
                                                      FreezeEmb=config_params.FreezeEmb)
        '''
        self.f_grad_shared, self.f_update = adadelta(lr0, self.CLM.tparams, grads_L2S,
                                              [CLM_srcx, CLM_srcx_mask, CLM_ctx_, classifier_x, classifier_mask, classifier_y, LMsores],
                                              [CLM_cost_avg, classifier_nlls_avg, consistent_loss, overall_L2S, norm_grads_L2S],
                                              )#self.classifier.tparams
    ...
    def train_one_minibatch(self, seqx, seqy, LMscore):
        ...
        CLM_cost_avg, classifier_nlls_avg, consistent_loss, overall_L2S, norm_grads_L2S  = self.f_grad_shared(
            CLM_x, CLM_xmask, labels, classifier_x, classifier_xmask, labels, LMscore
        )
        print 'CLM_cost_avg=%f, classifier_nlls_avg=%f, norm_grads_L2S=%f, consistent_loss=%f,' \
              ' overall_L2S=%f' % (
            CLM_cost_avg, classifier_nlls_avg, norm_grads_L2S, consistent_loss, overall_L2S )
        self.f_update(config_params.lrL2S)
        #self.f_update(config_params.lrL2S, config_params.lrS2L)

    def train(self):
        uidx = 0
        for eidx in xrange(0, config_params.maxEpoch):
            n_samples = 0
            print config_params.minibatch
